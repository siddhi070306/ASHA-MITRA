...
function App() {
  ...
  const handleLogin = async (e, overridePhone = null, overridePass = null) => {
    ...
    try {
      ...
      if (!response.ok) {
        // If server explicitly rejected credentials, show the server error
      ...
      // Offline fallback: check locally registered users if server is completely unreachable
      ...
      if (foundUser) {
        if (foundUser.password && foundUser.password === cleanPass) {
          ...
          showToast(`Welcome! Logged in offline as ${updatedUser.name}`);
          return;
        } else {
  ...
  // Merges API data with any locally-created records using deduplication by id/_id
  useEffect(() => {
    if (user) {
      // Helper: merge API data with local records, API data takes priority for duplicates
      const mergeRecords = (apiData, localData) => {
        const idMap = new Map();
        ...
        // API records overwrite local duplicates
      ...
      fetch(`${API_BASE_URL}/api/patients`)
        ...
        .then(data => {
          if (Array.isArray(data)) {
            setPatients(prev => mergeRecords(data, prev));
          }
        })
      ...
      fetch(`${API_BASE_URL}/api/triage`)
        ...
        .then(data => {
          if (Array.isArray(data)) {
            setTriageHistory(prev => mergeRecords(data, prev));
          }
        })
